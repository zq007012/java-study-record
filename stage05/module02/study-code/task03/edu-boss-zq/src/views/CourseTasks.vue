      ...
      <div class="inner" slot-scope="{ data, node }">
        ...
        <span class="actions">
          ...
          <el-button
            v-if="node.level == 1"
            size="small"
            @click.stop="handleNavigate('CourseTaskItem', data.id)"
          >
            课时管理
          </el-button>
          <!-- 课时管理按钮 -->
        ...
        <el-form-item label="状态变更为：">
          <el-select v-model="statusForm.status" style="width: 100%">
            <el-option
              ...
              :key="index"
              v-show="item != statusForm.data.status"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loading: false, //树形控件
      showAddSection: false, //添加或修改章节的dialog
  ...
  created() {
    ...
    //3.加载课程信息
    this.addSectionForm.course_id = id;
    this.loadCourse(this.addSectionForm.course_id);
  ...
  methods: {
    ...
    loadCourse(id) {
      return axios
        ...
        .then((response) => {
          this.addSectionForm.course_name = response.data.course_name;
        })
        .catch((error) => {
          this.$notify.error({
            title: "后台请求失败",
    ...
    handleShowAddSection() {
      ...
      this.addSectionForm.methodName = "saveOrUpdateSection";
    ...
    handleAddSection() {
      ...
      let action = this.addSectionForm.id ? "新建" : "编辑";
      return axios
        .post("/courseContent", this.addSectionForm)
        .then((response) => {
    ...
    handleEditSection(section) {
      this.addSectionForm.section_name = section.section_name;
      this.addSectionForm.description = section.description;
      this.addSectionForm.order_num = section.order_num;
    ...
    //方法7: 修改章节状态
    updateStatus() {
      if (this.statusForm.status == this.statusForm.data.status) {
        return (this.showStatusForm = false);
      }
    ...
    //8. 跳转到课时管理页面
    handleNavigate(routeName, sectionId) {
      this.$router.push({
        name: routeName,
        query: {
          courseId: this.addSectionForm.course_id,
          sectionId: sectionId,
        },
      });
    },
