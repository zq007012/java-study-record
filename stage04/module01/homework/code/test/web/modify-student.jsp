        ...
        <div class="from-group">
            ...
            <div id="gender" class="radio" style="margin-left: 2.5em">
                <label class="radio-inline">
                    <input name="gender" type="radio" value="男" checked="<%="男"==student.getGender()%>"/> 男
                </label>
                <label class="radio-inline">
                    <input name="gender" type="radio" value="女" checked="<%="男"==student.getGender()%>"/> 女
                </label>
            </div>
        </div>
...
<script>
    $(function () {
        <%
        // 从请求中获取'modifyFailed'属性, 如果有这个属性就说明是从ModifyServlet转发过来的
        // 就需要判断添加学生是否成功了
        boolean modifyFailed = request.getAttribute("modifyFailed") != null;
        if (modifyFailed){
        %>
        $(`#number`).data("unusableNumber",<%=student.getNumber()%>)
            ...
            if (null != unusableNumber && undefined != unusableNumber){
                if ( unusableNumber == $(this).val()){
                    $(this)
                        ...
                        .children(":first")
                        .children("span").text("已存在该学号,请换个学号");
                }else{
                    $(this)
