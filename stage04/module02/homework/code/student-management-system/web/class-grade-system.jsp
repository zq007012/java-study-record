...
<h1 class="text-center">班级信息管理系统</h1>
<hr/>

<div class="form-inline">
    <div class="form-group">
        <a class="btn btn-info" href="add-class-grade.jsp">新增</a>
    ...
    <div class="form-group">
        ...
        <input
                ...
                type="search"
                placeholder="请输入班级名称或者班主任姓名进行查询"
                class="form-control"
        />
    </div>
</div>
...
<div class="table-responsive">
    ...
    >
        ...
        <th class="text-center">班级口号</th>
        </thead>
        <c:set var="list" value="${sessionScope.classGradeList}"/>
        <c:choose>
        ...
        <c:set var="amountOfPage"
               value="${Math.ceil(amountOfClassGrade / 20)}"/>
        <c:set var="times" value="${indexOfPage -1}"/>
        <c:set var="startOfList" value="${20 * times}"/>
        <c:set var="endOfList"
               value="${Math.min(20 * indexOfPage -1 , amountOfClassGrade -1)}"/>
        <tbody id="userInfoTbody">

        <c:forEach var="classGrade" varStatus="id"
            ...
            <tr>
                ...
                <td class="text-center">${classGrade.classGradeName}
                </td>
                <td class="text-center">${classGrade.classTeacher}
                </td>
                <td class="text-center">${classGrade.classSize}
                </td>
                <td class="text-center">${classGrade.watchwords}
                </td>
            </tr>
        </c:forEach>
        </tbody>
...
<ul class="pagination" style="margin-top: 3em">
    <c:choose>
        ...
        <c:otherwise>
            <li>
                <a href="${'class-grade-system.jsp?indexOfPage='.concat(indexOfPage - 1)}"
                   aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
    ...
    <c:forEach var="i" begin="1" end="${amountOfPage}">
        <c:choose>
            ...
            <c:otherwise>
                <li>
                    <a href="${'class-grade-system.jsp?indexOfPage='.concat(i)}">
                            ${i}
                    </a>
                </li>
            </c:otherwise>
    ...
    $(function () {
        ...
        $("#searchBtn").click(function () {
            ...
            if (referenceIsEmpty(keyword)) {
                ...
                location.href = "show-all-class-grade";
            } else {
                //搜索框里有数据, 那就对该数据进行模糊搜索
                location.href = "search-class-grade?keyword=" + keyword;
            }
        });
        ...
        $("#warningConfirm").click(function () {
            ...
            if (!referenceIsEmpty(activatedByBtn) && "removeBtn" == activatedByBtn) {
                let selectedStudentIdArr = warningModal.data("selectedClassGradeIdArr");
                warningModal.removeData("activatedByBtn");
                warningModal.removeData("selectedClassGradeIdArr");
                location.href = "remove-class-grade?removedClassGradeIdArr=" + encodeURI(JSON.stringify(selectedStudentIdArr));
            }
        ...
        $("#removeBtn").click(function () {
            let selectedClassGradeIdArr = getSelectedClassGradeIdArr();
            let selectedCount = selectedClassGradeIdArr.length;
            if (selectedCount < 1) {
                showWarningModal("请至少勾选一条要删除的信息");
            } else {
                warningModal.data("activatedByBtn", "removeBtn");
                warningModal.data("selectedClassGradeIdArr", selectedClassGradeIdArr);
                showWarningModal("确定要删除勾选的" + selectedCount + "条数据吗?");
            }
        });
        //modify按钮的点击事件
        $("#modifyBtn").click(function () {
            let selectedStudentIdArr = getSelectedClassGradeIdArr();
            let selectedCount = selectedStudentIdArr.length;
            if (selectedCount == 1) {
                location.href = "modify-class-grade.jsp?" + "classGradeId=" + selectedStudentIdArr[0];
        ...
        //获取用户选择行的索引组成的数组
        function getSelectedClassGradeIdArr() {
